...
// AllUsers checks if there are any users in the database
...
func (m *postgresDBRepo) InsertRoomRestriction(res models.RoomRestriction) error {
	...
	stmt := `insert into room_restrictions (room_id, start_date, end_date, restrictions_id, 
		created_at, updated_at, reservations_id) 
...
// SearchAvailabilityByRoomID returns true if availability exists for a room,
// for roomID, and false if no availability
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	select 
		r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, 
		end_date, r.room_id, r.created_at, r.updated_at, 
		rm.id, rm.room_name
	from 
		reservations r
	join
...
// UpdateProcessedForReservation updates the processed field of a reservation
...
// GetRestrictionForRoomByDate returns all restrictions for a room on a specific date
