...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	A new reservation has been made from %s to %s.
	<br><br>
...
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	...
	for _, i := range rooms {
		m.App.InfoLog.Printf("Found room: %d | %s\n", i.ID, i.RoomName)
	}

	if len(rooms) == 0 {
		m.App.Session.Put(r.Context(), "error", "No availability found for the selected dates")
...
// ChooseRoom takes URL parameter id and redirects to make reservation
...
// BookRoom takes URL parameters s, e, id and calls the rendering method
...
// PostShowLogin handles Post request to the login page
...
// AdminShowReservation displays a reservation
...
func (m *Repository) AdminProcessReservation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println("Error processing reservation", err)
		return
	}
...
// AdminPostReservationsCalendar handles Post request to the reservations calendar
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	m.App.Session.Put(r.Context(), "flash", "Changes saved")
	http.Redirect(w, r, fmt.Sprintf("/admin/reservations-calendar?m=%d&y=%d", month, year), http.StatusSeeOther)
}
